...
use serenity::all::{Channel, ChannelType, Ready, VoiceState};
use serenity::async_trait;
use serenity::prelude::*;

use crate::db::Db;
use crate::{Config, LogUtil};
...
impl EventHandler for Handler {
    ...
    async fn voice_state_update(&self, ctx: Context, old: Option<VoiceState>, new: VoiceState) {
        ...
        let was_in_vc = old.and_then(|v| v.channel_id).map(|v| v.get() == self.config.vc_id.get()).unwrap_or(false);
        let now_in_vc = new.channel_id.map(|v| v.get() == self.config.vc_id.get()).unwrap_or(false);
